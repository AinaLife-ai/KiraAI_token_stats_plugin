...
- **修复 Gemini 400 报错（enum 空值）**：`query_token_stats` 工具的 `range`/`render` 参数 enum 数组含空字符串 `""`，Gemini 系列模型 function calling 规范不允许 enum 含空值 → 直接 400 拒绝（`enum[0]: cannot be empty`），导致该渠道所有模型调用失败。移除 enum 中的空字符串，`default` 兜底行为不变
