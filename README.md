...
- **余额探测并发与超时加固**（姐姐审计）：`_probe_all` 网络型源改为**并行探测**（单源 8s 超时、整体 15s 超时），不再串行 N×10s 拖住工具/命令查询；工具/命令/API 查询余额时若后台轮询正忙会**等待其完成**（最多 20s）再返回最新值，不再拿旧状态；模板源插入顺序修正（append 保持配置顺序，不再反转）
