...
> **v1.1.0**：新增 `newapi` 余额源类型（New-Api-User 头 + `/api/user/self` + quota 换算），余额显示单位可自定义（默认"元"）
...
| 💳 **余额监测** | 四类监测源：`auto`（按 URL 自动探测官方端点/One-API 中转站）、`custom`（自定义接口多端点尝试）、`newapi`（New-API 站点专属：New-Api-User 头 + `/api/user/self` + quota 换算）、`preset`（初始额度 − 已计费用）；余额单位可自定义（默认"元"） |
...
> 💡 余额单位：插件配置页 → 余额监测 → `balance_unit`（默认"元"），可改为"额度"、"美元"、"美刀"、"点数"等任意单位，工具/命令/WebUI 全局生效。
...
| 余额监测 | `balance_sources` | `[]` | 监测源数组（auto/custom/newapi/preset） |
...
- 配置文档与 README 同步更新（newapi 示例、单位说明、配置项总览）
