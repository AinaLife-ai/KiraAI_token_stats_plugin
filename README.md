...
> **v1.3.4**：「已进行」时长改天/时/分/秒完整组合显示（如 56天7时34分3秒）+ 数字缩写单位阶梯扩展为 K（千）/M（百万）/亿/B（十亿），前后端三处实现统一
> **v1.3.3**：修复挂件随机背景回归（伪元素 CSS 缺失导致不渲染）+ 主看板支持 🖼 自定义背景（与挂件共享图池/开关，不上传时线上随机背景完整保留）
>
> **v1.3.2**：余额源重复增殖修复（简易文本/快捷平台派生源标记+保存过滤+加载去重）+ 概览「已进行」秒级实时走时 + 派生源 WebUI 标注「由配置项管理」
>
> **v1.3.1**：维度分析费用修复 + 时间趋势四项修复（柱高比例/每日模型分色/漏天补齐/5分钟下钻）+ 挂件真·独立小窗（同源弹窗 + 折叠球单击展开 + 📌 PiP 一键置顶优先直开）+ 随机背景默认开（支持 🖼 自定义背景 1-10 张随机轮换）+ 性能优化（计算缓存/日志增量裁剪）+ 界面美化（骨架屏/数字滚动/卡片高光/页内确认）
>
